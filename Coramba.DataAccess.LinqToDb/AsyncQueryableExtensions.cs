...
namespace Coramba.DataAccess.LinqToDb
{
    public static class AsyncQueryableExtensions
    {
        ...
        public static async Task<ILoadWithQueryable<TEntity, TProperty>> LoadWith<TEntity, TProperty>(
            [NotNull] this Task<IQueryable<TEntity>> queryTask,
            [NotNull] Expression<Func<TEntity, TProperty>> navigationPropertyPath)
            where TEntity : class
            => (await queryTask).LoadWith(navigationPropertyPath);

        public static async Task<ILoadWithQueryable<TEntity, TProperty>> ThenLoad<TEntity, TPreviousProperty, TProperty>(
            [NotNull] this Task<ILoadWithQueryable<TEntity, IEnumerable<TPreviousProperty>>> queryTask,
            [NotNull] Expression<Func<TPreviousProperty, TProperty>> navigationPropertyPath)
            where TEntity : class
            => (await queryTask).ThenLoad(navigationPropertyPath);

        public static async Task<ILoadWithQueryable<TEntity, TProperty>> LoadWith<TEntity, TPreviousProperty, TProperty>(
            [NotNull] this Task<ILoadWithQueryable<TEntity, TPreviousProperty>> queryTask,
            [NotNull] Expression<Func<TPreviousProperty, TProperty>> navigationPropertyPath)
            where TEntity : class
            => (await queryTask).ThenLoad(navigationPropertyPath);

        public static async Task<IQueryable<TEntity>> ToQueryable<TEntity, TProperty>([NotNull] this Task<ILoadWithQueryable<TEntity, TProperty>> queryTask)
            where TEntity : class
            => await queryTask;
